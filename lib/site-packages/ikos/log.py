...
# yuna: utf8 to gbk
# must to gbk(redirect may be incorrect)
def printf_utf8_to_gbk(fmt, *args, **kwargs):

    if isinstance(fmt, str):
        fmt = fmt.decode('utf-8').encode(str_locale)
    elif isinstance(fmt, unicode):
        fmt = fmt.encode(str_locale)
    else:
        assert False,"Unvalid encoding format"

    new_args=[]
    for arg in args:
        if isinstance(arg, str):
            new_args.append(arg.decode('utf-8').encode(str_locale))
        elif isinstance(arg, unicode):
            new_args.append(arg.encode(str_locale))
        else:
            new_args.append(arg)
        
    file = kwargs.pop('file', sys.stdout)
    file.write(fmt % tuple(new_args) if new_args else fmt)
    file.flush()
