...
str_locale = locale.getdefaultlocale()[1]


def parse_arguments(argv):
    #usage = '%(prog)s [options] file[.c|.cpp|.bc|.ll]'
    usage = '%(prog)s [options] file[.c|.bc]'
    description = 'A Shared Data Analyzer for Interrupt-driven Embedded Software'
    ...
    analysis.add_argument('-e', '--entry-points',
                          dest='entry_points',
                          metavar='<function>',
                          help='List of program entry points (default: sum of task entry points and isr entry points)',
                          action='append')
    analysis.add_argument('-t-e', '--task-entry-points',
                          dest='task_entry_points',
                          metavar='<function>',
                          help='List of program task entry points (default: main)',
                          action='append')
    analysis.add_argument('-t-p', '--task-priority',
                          dest='task_priority',
                          metavar='<int>',
                          help='List of program task priority (default: 0)',
                          action='append')
    analysis.add_argument('-i-e', '--isr-entry-points',
                          dest='isr_entry_points',
                          metavar='<function>',
                          help='List of program isr entry points (default: isr)',
                          action='append')
    analysis.add_argument('-i-p', '--isr-priority',
                          dest='isr_priority',
                          metavar='<int>',
                          help='List of program isr priority (default: 1)',
                          action='append')
    ...
    #                       type=args.Integer(min=0))
    #
    analysis.add_argument('--all-entry-points',
                        dest='use_all_entry_points',
                        #help='Get and use all entry points',
                        action='store_true',
                        default=False)
    ...
    debug.add_argument('--temp-dir',
                       ...
                       default=None)
    # debug.add_argument('--output-dir',
    #                    dest='output_dir',
    #                    metavar='<directory>',
    #                    help='output directory',
    #                    default=None)
    # debug.add_argument('--save-all-data',
    #                    dest='save_all_data',
    #                    help='Save all the database',
    #                    action='store_true',
    #                    default=False)
    ...
    report.add_argument('-f', '--format',
                        ...
                        #                args.report_formats,
                        #                'auto'),
                        help='json     - Generate a json report',
    ...
    report.add_argument('--result',
                        dest='result_level',
                        metavar='',
                        help=args.help('Choose the level of result:',
                                       args.result_level,
                                       'all'),
                        choices=args.choices(args.result_level),
                        default='all')
    ...
    #if not opt.entry_points and not opt.use_all_entry_points:
    if not opt.entry_points:
        opt.entry_points = opt.task_entry_points + opt.isr_entry_points
        #opt.entry_points = ('main',)

    #by default, the task priority is 0
    if not opt.task_priority:
        opt.task_priority = ('0',)

    #by default, the isr priority is 1
    if not opt.isr_priority:
        opt.isr_priority = ('1',)
    progname = os.path.basename(argv[0])
    # if opt.entry_points and opt.use_all_entry_points:
    #     printf('%s:error: cannot use --entry-points and '
    #            '--all-entry-points',
    #            progname, file=sys.stderr)
    #     sys.exit(1)

    # verbosity changes the log level, if --log is not specified
    # if opt.save_all_data:
    #     file_name = str(uuid.uuid4())
    #     current_time = time.strftime("%Y%m%d-",time.localtime())
    #     opt.output_db = current_time + file_name + '.db'
    #
    # if opt.output_dir:
    #     real_path = os.path.abspath(opt.output_db)
    #     base_name = os.path.basename(opt.output_db)
    #     dir_name = os.path.dirname(real_path)
    #
    #     if (base_name != opt.output_db) and (dir_name != opt.output_dir):
    #         printf('error: output_dir:%s,output_db: %s\n', opt.output_dir,
    #               opt.output_db, file=sys.stderr)
    #         sys.exit(1)
    #
    #     opt.output_db = os.path.join(opt.output_dir, opt.output_db)
    #
    # verbosity changes the log level, if --log is not specified
...
def clang(
    ...
    #printf('Running %s' % command_string(cmd))
    subprocess.check_call(cmd)

def to_utf8(strs):
    utf8_strs = []
    for arg in strs:
        utf8_strs.append(arg.decode(locale.getdefaultlocale()[1]).encode("utf8"))
    return utf8_strs
...
def ikos_pp(pp_path, bc_path, entry_points, opt_level, inline_all, verify):
    ...
    cmd = [settings.ikos_pp(),
           '-opt=%s' % opt_level]
    if entry_points:
      cmd.append('-entry-points=%s' % ','.join(entry_points))
    ...
    log.debug('Running %s' % command_string(to_utf8(cmd)))
    #printf('%s' % cmd)
    subprocess.check_call(cmd)
...
def ikos_get_all_entry_points(db_path, pp_path, opt):
    # todo: should remove in future
    if os.path.isfile(db_path):
        os.remove(db_path)

    cmd = [settings.ikos_get_entry()]
    cmd += ['-log=none', pp_path]

    log.info('Getting all entry points')
    log.debug('Running %s' % command_string(to_utf8(cmd)))
...
def ikos_analyzer(db_path, pp_path, opt):
    ...
    #         '-d=%s' % opt.domain,
            '-a=sma',
            '-d=interval-congruence',
            '-entry-points=%s' % ','.join(opt.entry_points),
            '-task-entry-points=%s' % ','.join(opt.task_entry_points),
            '-task-priority=%s' % ','.join(opt.task_priority),
            '-isr-entry-points=%s' % ','.join(opt.isr_entry_points),
            '-isr-priority=%s' % ','.join(opt.isr_priority),
    ...
    cmd.append('-hardware-addresses=0x00000000-0xffffffff')
    # import options
...
def main(argv):
    ...
    #         sys.exit(e.returncode)
    #     try:
    #         with stats.timer('ikos-entry-point'):
    #             all_entry_points=ikos_get_all_entry_points(opt.output_db, pp_path, opt)
    #     except subprocess.CalledProcessError as e:
    #         printf('%s: error while getting all entry points\n', progname,  file=sys.stderr)
    #         sys.exit(e.returncode)
    #
    #     if len(all_entry_points) == 0:
    #         printf('%s: entry points is null, abort.\n', progname, file=sys.stderr)
    #         sys.exit(0)
    #     opt.entry_points = all_entry_points
    ...
    # output_db_path = (opt.output_db).decode(str_locale).encode("utf8")
    # db = OutputDatabase(path=output_db_path)
    #
    ...
    # if opt.display_times != 'no' and not opt.display_summary != 'sunwise':
    #     if not first:
    #         printf('\n')
    ...
    #
    # # display summary
    # if opt.display_summary != 'no' and not opt.display_summary != 'sunwise':
    #     if not first:
    #         printf('\n')
    ...
    #
    #     if opt.format == 'sunwise':
    #         # generate report
    #         from ikos import report_sunwise
    #
    #         rep = report_sunwise.generate_report(db,
    #                                         status_filter=opt.status_filter,
    #                                         analyses_filter=None)
    #
    #         # format report
    #         formatter_class = report_sunwise.formats[opt.format]
    ...
    #         formatter = formatter_class(opt.report_file, opt.report_verbosity)
    #         formatter.format(rep, opt.show_call_context)
    #
    # # close database
    # if opt.save_all_data:
    #     summary = report.generate_summary(db)
    #
    # # close database
    # db.close()
    #
    # if opt.remove_db and not opt.save_all_data:
    #     os.remove(opt.output_db)
    #
    # if opt.save_all_data:
    #     # make dir: error and warning
    #     error_db_dir = os.path.join(settings.BIN_DIR,'error_dbs')
    #     warning_db_dir = os.path.join(settings.BIN_DIR,'warning_dbs')
    #     mkdir_if_not_exists(error_db_dir)
    #     mkdir_if_not_exists(warning_db_dir)
    #
    #     if summary.error == 0 and summary.warning == 0:
    #         os.remove(opt.output_db)
    #     if summary.error != 0:
    #         shutil.move(opt.output_db, error_db_dir)
    #     else :
    #         shutil.move(opt.output_db, warning_db_dir)
    #
    #     with open('result.txt',"a") as fp:
    #         current_time = time.strftime("\n\n%Y-%m-%d %H:%M:%S\n", time.localtime())
    #         printf(current_time,file=fp)
    #         printf('database:%s\n', opt.output_db, file=fp)
    #         printf('errors:%s\n', str(summary.error), file=fp)
    #         printf('warnings:%s\n', str(summary.warning), file=fp)
